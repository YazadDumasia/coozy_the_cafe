...
class SingleChoiceDialogState extends State<CountryPickerDialog> {
  ...
  Widget _buildContent(BuildContext context) {
    return _filteredCountries.isNotEmpty
        ? ListView.builder(
            ...
            prototypeItem: SimpleDialogOption(
              child: Builder(
                builder: (context) {
        ...
        : widget.searchEmptyView ??
              Center(
                child: Text(
                  context.tr(
                        LocaleKeys.commonNoCountryFound,
  ...
  dynamic _buildHeader() {
    return widget.isSearchable
        ? Column(children: <Widget>[_buildTitle()!, _buildSearchField()])
        : _buildTitle();
  }

  Widget? _buildTitle() {
    return widget.titlePadding != null
        ? Padding(padding: widget.titlePadding!, child: widget.title)
        : widget.title;
  }
              ...
              .where(
                (Country country) => widget.searchFilter == null
                    ? country.name.toLowerCase().contains(
                          ...
                          country.iso3Code.toLowerCase().startsWith(
                            value.toLowerCase(),
                          )
