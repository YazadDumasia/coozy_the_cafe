...
class IndexBarDragNotifier implements IndexBarDragListener {
  @override
  final ValueNotifier<IndexBarDragDetails> dragDetails = ValueNotifier(
    IndexBarDragDetails(),
  );
}
...
///Default Index data.
const List<String> kIndexBarData = <String>[
  'A',
  'B',
  'C',
  'D',
...
class IndexBarOptions {
  ...
  const IndexBarOptions({
    ...
    this.indexHintTextStyle = const TextStyle(
      fontSize: 24.0,
      color: Colors.white,
    ),
...
class _IndexBarState extends State<IndexBar> {
  ...
  Widget _buildIndexHint(BuildContext context, String tag) {
    ...
    Widget child;
    final TextStyle textStyle = widget.options.indexHintTextStyle;
    final List<String> localImages = widget.options.localImages;
  ...
  Widget _buildItem(BuildContext context, int index) {
    final String tag = widget.data[index];
    ...
    if (widget.options.downItemDecoration != null) {
      ...
      decoration = (selectIndex == index)
          ? widget.options.selectItemDecoration
          : null;
