...
class MaskTextInputFormatter implements TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    final String replacementText = afterText.substring(
      afterChangeStart,
      afterChangeEnd,
    );
    ...
    if (beforeResultTextLength == 0 && _resultTextArray.length > 1) {
      for (int i = 0; i < mask.length; i++) {
        if (_maskChars.contains(mask[i])) {
          final List<String> resultPrefix = _resultTextArray._symbolArray
              .take(i)
              .toList();
          for (int j = 0; j < resultPrefix.length; j++) {
    ...
    final int finalCursorPosition = cursorPos < 0
