...
void main() {
  ...
  setUp(() async {
    ...
    repository = InvoiceManagementRepositoryImpl(
  ...
  test(
    'Updating invoice in InvoicesDao updates invoicesTable, invoiceItems, ordersTable, and orderItemsTable',
    () async {
      ...
      final dbInvoiceItems = await invoicesDao.getInvoiceItemsByInvoiceId(
        invoiceId,
      );
      ...
      expect(
        orderItems.any(
          (i) => i.itemId == 10 && i.quantity == 3 && i.sellingPrice == 50.0,
        ),
        isTrue,
      );
      expect(
        orderItems.any(
          (i) => i.itemId == 20 && i.quantity == 1 && i.sellingPrice == 80.0,
        ),
        isTrue,
      );
    },
  ...
  test(
    'Updating invoice via InvoiceManagementRepositoryImpl syncs both invoice and order records',
    () async {
      // 1. Create linked order
      final orderId = await ordersDao.createNewOrder(
        order: OrdersTableCompanion(
