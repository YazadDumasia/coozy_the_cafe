...
void main() {
  ...
  test(
    'Soft delete invoice and linked order marks isDeleted=true in both tables',
    () async {
      // 1. Create an order
      ...
      final initialInvoices = await invoicesDao.getInvoicesPaginated(
        limit: 10,
        pageNo: 1,
      );
      ...
      final postInvoices = await invoicesDao.getInvoicesPaginated(
        limit: 10,
        pageNo: 1,
      );
  ...
  test('Deleting order also soft-deletes linked invoice in database', () async {
    ...
    final activeInvoices = await invoicesDao.getInvoicesPaginated(
      limit: 10,
      pageNo: 1,
    );
